...
Projectile::Projectile(sf::Vector2f location, sf::Vector2f size, sf::Sprite*sprite, int ownerType, int speed): Entity(location, size, sprite){
    this->location = location;
    this->ownerType = ownerType;
    this->speed = speed;
}

bool Projectile::checkCollision(Entity Collider)
{
...
sf::Vector2f Projectile::get_location()
{
    return this->location;
}

sf::Vector2f Projectile::get_size()
{
    return this->size;
}
...
int Projectile::get_ownerType(){
    return ownerType;
}

//moves the projectile up or down the screen depending on whether the projectile is owned by the player or an enemy, this function is run every frame
